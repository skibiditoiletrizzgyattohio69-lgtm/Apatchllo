...
Apatchllo is a Windows agent written in C# using the 4.0 .NET Framework designed to be used in SpecterOps training offerings.

## Installation
To install Apatchllo, you'll need Mythic installed on a remote computer. You can find installation instructions for Mythic at the [Mythic project page](https://github.com/its-a-feature/Mythic/).

From the Mythic install directory, use the following command to install Apatchllo as the **root** user:

```
./mythic-cli install github https://github.com/skibiditoiletrizzgyattohio69-lgtm/Apatchllo.git
```

From the Mythic install directory, use the following command to install Apatchllo as a **non-root** user:

```
sudo -E ./mythic-cli install github https://github.com/skibiditoiletrizzgyattohio69-lgtm/Apatchllo.git
```
...
The HTTP profile calls back to the Mythic server over the basic, non-dynamic profile. When selecting options to be stamped into Apatchllo at compile time, all options are respected with the exception of those parameters relating to GET requests.
...
Apatchllo can route SOCKS traffic regardless of what other commands are compiled in. To start the socks server, issue `socks -Port [port]`. This starts a SOCKS server on the Mythic server which is `proxychains4` compatible. To stop the SOCKS proxy, navigate to the SOCKS page in the Mythic UI and terminate it.
...
![ls browserscript](documentation-payload/Apatchllo/commands/images/ls02.png)

Interfaces with Mythic's filebrowser and caches data server-side:
![ls mythic builtin](documentation-payload/Apatchllo/commands/images/filebrowser.png)
...
![injection](documentation-payload/Apatchllo/commands/images/get_injection_techniques.png)
...
![jobs](documentation-payload/Apatchllo/commands/images/jobs.png)
...
![artifacts](documentation-payload/Apatchllo/commands/images/artifacts.png)
